...
@section ('section-js')
	...
	<script>
		...
		$("#image").fileinput({
		    ...
		    defaultPreviewContent: '<img src="@if($data->image != '') {{ asset('public/uploads/slide/image/'.$data->image) }} @else http://via.placeholder.com/1903x700 @endif" alt="Missing Image" class="img img-responsive"><span class="text-muted">Click to select <br /><i style="font-size:12px">Image dimesion must be 1903x700 with .jpg or .png type</i></span>',
	...
	<form id="form" action="{{ route($route.'.update') }}" name="form" method="POST"  enctype="multipart/form-data">
		...
		<div class="form-group row">
			...
			<div class="col-sm-10">
				...
				@else 
				<button type="button" onclick="deleteConfirm('{{ route($route.'.deleted', $data->id) }}', '{{ route($route.'.index') }}')" class="btn btn-danger"> <fa class="fa fa-trash"></i> Delete</button>
				@endif
			</div>
		</div>
	</form>
